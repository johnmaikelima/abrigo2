...
export async function GET(request: NextRequest) {
  try {
    // Garantir que a conexão com o banco de dados seja estabelecida
    await connectToDatabase();
    
    // Buscar configurações ou criar se não existirem
    let settings;
    try {
      settings = await BlogSettingsModel.findOne();
      
      // Se não existir, criar um novo documento de configurações
      if (!settings) {
        ...
        settings = new BlogSettingsModel({
          ...
          customHtml: { head: '', body: '' },
          socialMedia: {}
        });
        await settings.save();
      }
    } catch (dbError) {
      console.error('Erro ao buscar/criar configurações:', dbError);
      // Criar um objeto de configurações em memória para evitar erro
      settings = {
        name: 'Meu Blog',
        description: 'Descrição do meu blog',
        customHtml: { head: '', body: '' },
    ...
    // Garantir que settings não seja undefined ou null
    if (!settings) {
      settings = {
        name: 'Meu Blog',
        description: 'Descrição do meu blog'
    ...
    // Mesmo em caso de erro, retornar algum dado válido para evitar erro de JSON
    return NextResponse.json({
      success: false,
      error: 'Erro ao obter configurações do blog',
